...
def extract_has_real_world_text(text: str) -> bool:
    text_lower = text.lower()
    return any(kw in text_lower for kw in REAL_WORLD_KEYWORDS)


def _extract_method_names_from_text(text: str, paper_title: str) -> list[str]:
...
def extract_method_names(html: str, paper_title: str) -> list[str]:
    ...
    return _extract_method_names_from_text(strip_tags(html), paper_title)


def build_fallback_method_summary(abstract: str, max_chars: int = 500) -> str:
    text = re.sub(r"\s+", " ", abstract).strip()
    if not text:
        return ""
    if len(text) <= max_chars:
        return text
    end = text.rfind(". ", 250, max_chars + 40)
    if end > 0:
        return text[:end + 1]
...
async def enrich_one(paper: dict, sem: asyncio.Semaphore) -> dict:
    ...
    if not arxiv_id:
        ...
        result["section_headers"] = paper.get("section_headers", [])
        result["captions"] = paper.get("captions", [])
        result["has_real_world"] = extract_has_real_world_text(combined_text)
        result["method_names"] = _extract_method_names_from_text(combined_text, title)
        result["method_summary"] = build_fallback_method_summary(abstract)
        return result
